...
| **Execution Hooks** | Defined in `hooks/hooks.json` (trigger: `SessionStart`). Runs shell commands. | Defined in `hooks/hooks.json` (triggers: `PreInvocation`, `PostToolUse`, `Stop`) via native Antigravity named dictionary schema. | **100% Compatible** | `scripts/compile_prompts.py` compiles `core/hooks/hooks.spec.json` into Antigravity's native `hooks/hooks.json` with inline `AGY_PROJECT_DIR`/`AGY_PLUGIN_ROOT` translation (origin: #173, #193). |
