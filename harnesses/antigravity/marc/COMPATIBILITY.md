...
The mARC team is designed to be harness-agnostic. The core specialist prompts ([AGENTS.md](file:///home/ubuntu-24/repos/NexaDuo/mARC/AGENTS.md)) and leader skills ([SKILL.md](file:///home/ubuntu-24/repos/NexaDuo/mARC/harnesses/claude-code/marc/skills/tech-lead/SKILL.md)) contain role behaviors and boundaries rather than vendor-specific runtime logic. However, the executing harness dictates manifest schemas, slash command formats, subagent execution APIs, and local file paths.
...
| **Local Config Path** | Discovers workspace configurations under `.claude/` (e.g., [team.toml](file:///home/ubuntu-24/repos/NexaDuo/mARC/.claude/team.toml)). | Discovers workspace configurations under `.agents/` (e.g., `.agents/team.toml`). | **Requires Dual-Support** | Update config search scripts to fall back to `.agents/` when `.claude/` is missing. |
...
- [x] Create Antigravity manifest `plugin.json` ([harnesses/antigravity/marc/plugin.json](file:///home/ubuntu-24/repos/NexaDuo/mARC/harnesses/antigravity/marc/plugin.json))
- [x] Create `COMPATIBILITY.md` tracker ([harnesses/antigravity/marc/COMPATIBILITY.md](file:///home/ubuntu-24/repos/NexaDuo/mARC/harnesses/antigravity/marc/COMPATIBILITY.md))
