...
| **Execution Hooks** | Defined in `hooks/hooks.json` (trigger: `SessionStart`). Runs shell commands. | Defined in `hooks.json` at plugin root (triggers: `PreInvocation`, `PostToolUse`, `Stop`) via native Antigravity named dictionary schema. | **100% Compatible** | `scripts/compile_prompts.py` compiles `core/hooks/hooks.spec.json` into Antigravity's native `hooks.json` at plugin root. Runtime findings: Antigravity does not export `AGY_PLUGIN_ROOT`, but executes hooks with `$PWD` set to the plugin root and `$OLDPWD` set to the workspace; hooks fall back to `${AGY_PLUGIN_ROOT:-$PWD}` and `${AGY_PROJECT_DIR:-${OLDPWD:-$PWD}}`. `PreInvocation` fires per-turn rather than session-start; session extraction falls back from `conversationId` / `$ANTIGRAVITY_CONVERSATION_ID` to `$PPID` (origin: #173, #193, #197, #220, #221). |
...
- [x] Native Antigravity hook dialect compiler (`hook_dialect: "antigravity"`) targeting `PreInvocation`, `PostToolUse`, `Stop` with `$PWD`/`$OLDPWD` fallbacks and session/conversation ID extraction (origin: #193, #220, #221)
