...
`.claude` for this harness (it does not for Antigravity, where both
resolve to `.agents` — skip this step there, there is nothing to migrate) and,
if so, whether `$ROOT/.claude/team.toml` already exists. If it does:
...
- On an explicit "yes", write it to the new path and **offer to delete the
  obsolete `.claude/team.toml`** — leaving it only re-triggers the
