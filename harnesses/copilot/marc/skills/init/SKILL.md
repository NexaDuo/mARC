...
literal strings `.agents` and `.github/copilot`. **If they are the
...
Having confirmed `.agents` and `.github/copilot` are genuinely
different paths for this harness, check whether `$ROOT/.github/copilot/team.toml`
...
- On an explicit "yes", write it to the new path and **offer to delete the
  obsolete `.github/copilot/team.toml`** — but only because the gate above
  already established `.github/copilot` and `.agents` are two
