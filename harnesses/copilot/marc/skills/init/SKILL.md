...
legacy one (`.github/copilot/team.toml`) — checking only the new path misses a
pre-existing legacy file entirely and silently produces two live configs.

**Legacy migration (`.github/copilot/team.toml` → `.agents/team.toml`):**
before composing a fresh file, check whether `.agents` differs from
`.github/copilot` for this harness (it does not for Antigravity, where both
resolve to `.agents` — skip this step there, there is nothing to migrate) and,
if so, whether `$ROOT/.github/copilot/team.toml` already exists. If it does:
...
- On an explicit "yes", write it to the new path and **offer to delete the
  obsolete `.github/copilot/team.toml`** — leaving it only re-triggers the
