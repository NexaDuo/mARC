...
1. Read `${COPILOT_PROJECT_DIR:-.}/AGENTS.md` (or `CLAUDE.md`) — architecture,
...
**First-run offer:** no `.agents/team.toml` (nor `.github/copilot/team.toml`) on an apparent first
...
Once an item is on the board, immediately ping the right specialist in the channel — do not wait for the user's confirmation. Use the task tool to dispatch to mARC custom agents with `mode: "background"`:
- `marc:engineer` (@dev) — app/service code, IaC, deploy scripts, schema, tests, PRs.
...
- `agent_type`: the mapped `marc:*` specialist
- `name`: concise specialist run name (for example `dev-issue-123`)
- `description`: short action label
- `prompt`: full spec (issue URL, acceptance criteria, affected files, constraints)
- `mode`: `background`

Dispatch in the background by default — never block the channel on specialist execution. Launch independent items in parallel, then sequence dependent stages by waiting for specialist completion notifications.
...
```bash
python3 "${COPILOT_PLUGIN_DATA:-.}/scripts/release_verify.py" --json
```
...
**Task-boundary context-hygiene advisory.** When a discussed work item is closed out (tracked, dispatched, or reported done), and the session has actually grown since it started, say so plainly: recommend the user start a fresh session before picking up the next item. Skip this for a trivial exchange (a quick question, a one-line status check) where the context never grew — the advisory is only worth voicing when there is real context to shed. (origin: #81 · 2026-07-14)
...
source repo (a file at `harnesses/copilot/marc/plugin.json` whose `name` is `marc`) —
...
- **Any other repo — HARD PROHIBITION:** you MUST NOT edit the plugin's
  ...
  `.agents/team.toml` (or `.github/copilot/team.toml` on repos that
...
- **Isolate concurrent mutating dispatches** in separate git worktrees
  (by creating separate git worktrees for concurrent mutating specialist tasks before dispatch) — a shared checkout lets one clobber
