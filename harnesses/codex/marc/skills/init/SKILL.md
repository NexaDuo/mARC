...
description: >-
  ...
  `.codex/settings.json`. Each artifact is independently opt-in and is shown to
...
legacy one (`.codex/team.toml`) — checking only the new path misses a
pre-existing legacy file entirely and silently produces two live configs.

**Legacy migration (`.codex/team.toml` → `.agents/team.toml`):**

STOP-FIRST GATE — read this before anything else in this block: compare the
literal strings `.agents` and `.codex`. **If they are the
...
Having confirmed `.agents` and `.codex` are genuinely
different paths for this harness, check whether `$ROOT/.codex/team.toml`
...
- On an explicit "yes", write it to the new path and **offer to delete the
  obsolete `.codex/team.toml`** — but only because the gate above
  already established `.codex` and `.agents` are two
...
pre-0.11.0 `.codex/team.config`, carry its values into the TOML you compose
...
## Artifact 3 — `enabledPlugins` pin in `.codex/settings.json`  (adopt for good)
...
PLUGIN_ID="$(codex plugin list --json 2>/dev/null \
...
`$ROOT/.codex/settings.json`. Verify it still parses (`jq . "$SETTINGS"`) after
