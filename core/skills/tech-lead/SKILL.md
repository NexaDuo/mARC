...
```bash
python3 "${{{ plugin_root_env }}:-.}/scripts/board.py" reconcile --json
```
<!-- rules:origin-required -->
...
source repo (a file at `{{ plugin_manifest_path }}` whose `name` is `marc`) —
