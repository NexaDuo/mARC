...
1. Read `${{{ project_dir_env }}:-.}/AGENTS.md` (or `CLAUDE.md`) — the repo's
...
2. Read `${{{ project_dir_env }}:-.}/{{ config_dir }}/team.toml` if present — it pins the gh
   ...
   context. If only the legacy `{{ config_dir }}/team.config` exists, tell the user once:
...
If **both** `AGENTS.md` **and** `{{ config_dir }}/team.toml` are absent, this repo has no
...
> `{{ config_dir }}/team.toml` (and optionally a lean `AGENTS.md` skeleton) so the board,
...
CFG="${{{ project_dir_env }}:-.}/{{ config_dir }}/team.toml"
...
[ ! -f "$CFG" ] && [ -f "${{{ project_dir_env }}:-.}/{{ config_dir }}/team.config" ] \
  && echo "DEPRECATED: {{ config_dir }}/team.config -> re-run /marc:init to migrate to {{ config_dir }}/team.toml"
...
```bash
python3 "${{{ plugin_root_env }}:-.}/scripts/board_reconcile.py" set-status \
  --issue <N> --status "<Todo|In Progress|Blocked|Done>"
```
It reads all repo facts from `{{ config_dir }}/team.toml` at runtime (same
zero-dependency fallbacks as reconciliation — no hardcoded org/repo/board),
...
a call or token threshold. Run `python3 "${{{ plugin_root_env }}:-.}/scripts/token_sentinel.py"`
...
{{ compact_action }} or a Sonnet drop, debounced to once per threshold band per turn. It NEVER
...
{{ compact_action }} first) rather than toggling turn-by-turn. A warn-only `PostToolUse`
...
It reads all repo facts from `{{ config_dir }}/team.toml` at runtime (with the
...
cache (e.g., `~/{{ config_dir }}/plugins/...` or equivalent) that is a no-op to edit and is overwritten
on the next update.
...
`{{ plugin_manifest_path }}` whose `name` is `marc`
...
PLUGIN_MANIFEST="${{{ project_dir_env }}:-.}/{{ plugin_manifest_path }}"
...
- **Flush (batched, periodic):** roll the buffer into the appropriate versioned
  ...
  otherwise the **consuming repo's AGENTS.md / {{ config_dir }}/team.toml** — in **one PR** when
...
- **Isolate concurrent mutating dispatches.** Specialists that WRITE files in parallel
  must each run in their own git worktree ({{ isolation_instructions }});
