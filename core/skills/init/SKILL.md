...
This check MUST cover both the new path (`{{ agents_dir }}/team.toml`) and the
legacy one (`{{ config_dir }}/team.toml`) — checking only the new path misses a
pre-existing legacy file entirely and silently produces two live configs.

**Legacy migration (`{{ config_dir }}/team.toml` → `{{ agents_dir }}/team.toml`):**
before composing a fresh file, check whether `{{ agents_dir }}` differs from
`{{ config_dir }}` for this harness (it does not for Antigravity, where both
resolve to `.agents` — skip this step there, there is nothing to migrate) and,
if so, whether `$ROOT/{{ config_dir }}/team.toml` already exists. If it does:
- Show the user its content and tell them plainly what you're about to do:
  move it to `$ROOT/{{ agents_dir }}/team.toml` (same schema, only the location
...
- On an explicit "yes", write it to the new path and **offer to delete the
  obsolete `{{ config_dir }}/team.toml`** — leaving it only re-triggers the
  SessionStart deprecation notice on every session and risks it drifting out of
  sync with the new file it no longer feeds.
- Never write a fresh `{{ agents_dir }}/team.toml` from discovered facts while
