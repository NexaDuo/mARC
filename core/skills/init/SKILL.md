...
literal strings `{{ agents_dir }}` and `{{ config_dir }}`. **If they are the
...
Having confirmed `{{ agents_dir }}` and `{{ config_dir }}` are genuinely
different paths for this harness, check whether `$ROOT/{{ config_dir }}/team.toml`
...
- On an explicit "yes", write it to the new path and **offer to delete the
  obsolete `{{ config_dir }}/team.toml`** — but only because the gate above
  already established `{{ config_dir }}` and `{{ agents_dir }}` are two
  ...
  it re-triggers a notice (it doesn't: once `{{ agents_dir }}/team.toml`
